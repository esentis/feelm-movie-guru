...
typedef PickerSelectedCallback = void Function(
    CustomPicker picker, int index, List<int> selecteds);

/// Picker confirm callback.
typedef PickerConfirmCallback = void Function(
    CustomPicker picker, List<int> selecteds);
          ...
          if (_cancelText != null && _cancelText != '') {
            actions.add(TextButton(
                ...
                child: cancelTextStyle == null
                    ? Text(_cancelText)
                    : DefaultTextStyle(
                        style: cancelTextStyle!, child: Text(_cancelText))));
          }
        } else {
          actions.add(cancel!);
        }
...
class PickerWidgetState<T> extends State<_PickerWidget> {
  ...
  Widget _buildHeaderButton(BuildContext context,
      ...
      TextStyle? style}) {
    return TextButton(
        style: CustomPicker._getButtonStyle(ButtonTheme.of(context)),
        onPressed: onPressed,
        child: Text(text,
  ...
  List<Widget> _buildViews() {
            ...
            child: _wait
                ...
                : StatefulBuilder(
                    builder: (context, state) {
                      ...
                      // 上一次是空列表
                      var _lastIsEmpty = scrollController[i].hasClients &&
    ...
    if (picker.delimiter != null && !_wait) {
      for (var i = 0; i < picker.delimiter!.length; i++) {
        ...
        var item = Container(height: picker.height, child: o.child);
        if (o.column < 0) {
          items.insert(0, item);
...
abstract class PickerAdapter<T> {
  ...
  Widget makeTextEx(
    ...
    double? _txtSize = CustomPicker.DefaultTextSize;
...
class PickerDataAdapter<T> extends PickerAdapter<T> {
  ...
  Widget buildItem(BuildContext context, int index) {
    ...
    var isSel = index == picker!.selecteds[_col];
    if (item.text != null) {
...
class NumberPickerAdapter extends PickerAdapter<int> {
  ...
  void initSelects() {
    ...
    if (picker!.selecteds.length == 0) {
      for (var i = 0; i < _maxLevel; i++) {
        var v = data[i].indexOf(data[i].initValue);
        if (v < 0) v = 0;
...
class DateTimePickerAdapter extends PickerAdapter<DateTime> {
  ...
  Widget buildItem(BuildContext context, int index) {
    var _text = '';
    var colType = getColumnType(_col);
    switch (colType) {
      case 0:
        if (twoDigitYear) {
      ...
      case 4:
        ...
        break;
      case 6:
        var _ampm = strAMPM ?? PickerLocalizations.of(context).ampm;
  ...
  void doSelect(int column, int index) {
    ...
    var __day = _calcDateCount(year, month);
  ...
  int _getAPColIndex() {
    var items = customColumnType ?? columnType[type];
